...
function initFirebase() {
  try {
    ...
    db.ref('specimens').on('value', snap => {
      ...
      // Don't disturb the modal if user is currently filling it in
      const modalOpen = document.getElementById('delivery-modal') &&
                        document.getElementById('delivery-modal').classList.contains('open');
      if (modalOpen) {
        // Just update autocomplete silently, don't re-render or reset form
        updateAutocomplete();
        return;
      }
...
function saveDelivery(){
  ...
  saveSpecimenToDb(entry)
    .then(()=>{
      closeDeliveryModal();
      // Keep the month of the saved entry visible
