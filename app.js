...
function getNumDaysFromLabel(label) {
  if (!label) return 0;
...
function computeStatusFor(dueDate, testDate) {
  ...
  if (testDate.length < 8 || dueDate.length < 8) return 'Pending';
...
function initFirebase() {
  try {
    ...
    db.ref('specimens').on('value', snap => {
      ...
      // Strictly block ALL UI updates when modal is open
      if (modalIsOpen) {
        // Still save data locally but don't touch the UI
        return;
      }
...
/* clearAllData removed - use per-entry delete instead */
...
function renderOverall(md) {
  const tot    = md.length;
  // Recompute status live for KPI accuracy
  const ontime = md.filter(d=>{
    if (!d.dateTesting) return false;
    const due = d.dueDate || computeDueDateFor(d.dateCasted, getNumDaysFromLabel(d.numDays));
    return computeStatusFor(due, d.dateTesting) === 'On Time';
  }).length;
  const late = md.filter(d=>{
    if (!d.dateTesting) return false;
    const due = d.dueDate || computeDueDateFor(d.dateCasted, getNumDaysFromLabel(d.numDays));
    return computeStatusFor(due, d.dateTesting) === 'Late';
  }).length;
  const pend   = md.filter(d=>!d.dateTesting||!d.dateTesting.length).length;
...
function renderLog() {
  ...
  let rows=[...md].sort((a,b)=>{
    // Latest Date Testing on top; if no test date, use Date Casted
    const da = b.dateTesting||b.dateCasted||'';
    const db2 = a.dateTesting||a.dateCasted||'';
  ...
  if(!rows.length){
    ...
    tbody.innerHTML=rows.map(function(d){
      ...
      // Recompute dueDate on the fly if not saved, then recompute status
      const liveDue = d.dueDate || computeDueDateFor(d.dateCasted, getNumDaysFromLabel(d.numDays));
      const hasTestDate = d.dateTesting && d.dateTesting.length > 0;
      const liveStatus = hasTestDate
        ? computeStatusFor(liveDue, d.dateTesting)
        : 'Pending';
      const pc=liveStatus==='On Time'?'p-pass':liveStatus==='Late'?'p-rej':'p-pend';
      const isOver=liveStatus==='Pending'&&d.dueDate&&d.dueDate<todayStr;
      ...
      return '<tr'+op+'>'+
        ...
        '<td><span class="pill '+pc+'" style="'+(liveStatus==='On Time'?'background:#EAF3DE;color:#3B6D11':liveStatus==='Late'?'background:#FCEBEB;color:#A32D2D':'background:#FAEEDA;color:#854F0B')+'">'+liveStatus+'</span>'+
