...
const DAYS_LIST = ['01 Days','03 Days','07 Days','14 Days','28 Days'];
const DAYS_NUM  = {'01 Days':1,'03 Days':3,'07 Days':7,'14 Days':14,'28 Days':28};
...
function computeDueDateFor(castedDate, daysLabel) {
  if (!castedDate || !daysLabel) return '';
  const num = DAYS_NUM[daysLabel];
  if (!num) return '';
  return addWorkingDays(castedDate, num);
}
...
function computeStatusFor(dueDate, testDate) {
  ...
  return testDate <= dueDate ? 'On Time' : 'Late';
}
  ...
  try {
    ...
    db.ref('specimens').on('value', snap => {
      ...
      specimens = val
        ...
        : [];
      specimens.sort((a,b) => (b.dateCasted||'').localeCompare(a.dateCasted||''));
      saveLocal();
      buildMonthSelect();
      render();
...
function buildMonthSelect() {
  ...
  if (!sel) return;
  const cur = sel.value;
  const months = new Set(specimens.map(d => (d.dateCasted||'').slice(0,7)).filter(Boolean));
  const now = new Date();
...
function monthSpecimens() { return specimens.filter(d=>(d.dateCasted||'').startsWith(selMonth())); }
...
function computeDueDate(){
  ...
  const days=getVal('m-days');
  setVal('m-due', computeDueDateFor(casted,days));
  computeAgeAndStatus();
}
...
function updateBranches(selected) {
  ...
  if (!sel) return;
  const branches = BRANCHES[center] || [];
...
function openAddForm(){
  try{
    ...
    setVal('m-testcenter','');setVal('m-branch','');
    setVal('m-coordinator','');setVal('m-coordinator-other','');
    const cw=document.getElementById('coord-other-wrap'); if(cw) cw.style.display='none';
...
function openEditForm(idx){
  try{
    ...
    setVal('m-testcenter', d.testCenter||'');
...
function saveDelivery(){
  ...
  const dueDate   =getVal('m-due')||computeDueDateFor(casted,numDays);
  ...
  const entry={
    ...
    numDays,     dateCasted:casted,
    dateTesting, age, status,
    testCenter:  getVal('m-testcenter'),
    branch:      getVal('m-branch'),
  ...
  saveSpecimenToDb(entry)
    .then(()=>{
      closeDeliveryModal();
      setVal('sel-month',casted.slice(0,7));
      buildMonthSelect();
      render();
      if(activeTab==='kpi') setTimeout(renderCharts,80);
